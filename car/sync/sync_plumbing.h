 ...
 * 3. We assume that the charbuf definition is the common NDN one.
 ...
 *    access must go through the normal NDN interfaces.
...
#ifndef NDN_SyncDepends
#define NDN_SyncDepends
...
struct sync_plumbing {
    struct ndn *ndn;                // ndn handle to share
    struct ndn_schedule *sched;     // scheduler to share
...
struct sync_plumbing_client_methods {
    ...
    // returns > 0 if newly stored
    // method is NULL if no local store
    enum ndn_upcall_res (* r_sync_upcall_store)(struct sync_plumbing *sd,
